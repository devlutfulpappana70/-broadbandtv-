...
global $product;
    ...
    foreach ( $attachment_ids as $attachment_id ){
        ...
        if( !empty($image_attributes[0]) ){
            $id = 'a'.$i;
            $html .=  '
                <div class="tab-pane fade '.$class.'" id="pd-'.$id.'" role="tabpanel" aria-labelledby="pd-'.$id.'-tab">
                    <div class="big-img">';
                    $html .= '<img src="'.esc_url($image_attributes[0]).'" alt="'.get_post_meta( attachment_url_to_postid($image_attributes[0]), '_wp_attachment_image_alt', true).'">';
                $html .= '</div>
                </div>
            ';
            $i++;
...
?>
	...
	<?php do_action( 'telnet_woocommerce_product_thumbnails' ); ?>
