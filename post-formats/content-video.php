 ...
 */
    ...
    $id = get_the_ID();
    $post_video_metabox = get_post_meta($id, 'telnet_post_video_meta', true) ? get_post_meta($id, 'telnet_post_video_meta', true) : [];
...
?>
    ...
    <article id="post-<?php the_ID(); ?>"  <?php post_class( 'tx-blog-box tx-blogDetails-box'); ?>>
        ...
        <div class="tx-blog-box__tx-thumb tx-thumb-zoom pb-30 position-relative">
            <div class="thumb-wrapper tx-radious-25">
                <?php the_post_thumbnail( 'full', ['class' => 'img-responsive w-100'] ); ?>
            </div>

            <?php if(!empty( $post_video_link )) : ?>
            <a href="<?php print esc_url( $post_video_link );?>" data-rel="lightcase" class="tx-round-btn tx-video-btn position-absolute top-50 start-50 translate-middle">
    ...
    <article id="post-<?php the_ID(); ?>"  <?php post_class( 'tx-blog-box tx-radious-25 tx-border-all tx-border-op-10 mt-30 format-video'); ?>>

        <?php if ( has_post_thumbnail() ): ?>
        <div class="tx-blog-box__tx-thumb tx-thumb-zoom tx-pd-30 pb-0 position-relative">
            <a class="thumb-wrapper tx-radious-25" href="<?php the_permalink(); ?>">
                <?php the_post_thumbnail( 'full', ['class' => 'img-responsive w-100'] ); ?>
            </a>
